...
float densiteTerrain(Terrain *T)
{
  int nbObstacles;
  float dObtenue;
  nbObstacles=nb_cases_occupees(T);
  dObtenue=((float)nbObstacles)/(largeur(T)*hauteur(T));
  return dObtenue;
}
...
int main(int argc, char **argv) {
  ...
  float dTotale, dMoyenne, pourcentageTerrainsValides, nbTerrainsGeneres = 0.0;
  ...
  for(i = 0; i < N; i++)
  {
    ...
    fprintf(resFile,"Densite d'obstacle obtenue: %f\n\n",densiteTerrain(&T));
    dTotale = dTotale + densiteTerrain(&T);
  }

  dMoyenne=(dTotale)/N;
  pourcentageTerrainsValides=(N/nbTerrainsGeneres)*100;
  fprintf(resFile,"Densite d'obstacle moyenne : %f\n",dMoyenne);
  fprintf(resFile,"Pourcentage de terrains valides : %f%% \n",pourcentageTerrainsValides);
